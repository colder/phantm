...
\begin{itemize}
  ...
  \item Objects: Objects are represented differently than other types. The reason
    ...
    \begin{itemize}
          \item \verb=TObjectRef=: Object reference. This is the type to which
            any well-defined object value is assigned.
          \item \verb=RealObjectType=: Real object type pointed to by the
...
\begin{itemize}
  \item \verb=TAnyArray=: we know nothing about the content of this array, any
    lookup will return the type \verb=TAny=.
  \item \verb=TPreciseArray=: we keep key-type associations for every entries
    in that array. This array is completely determined. A lookup to a key not
    defined will generate a notice.
  \item Polluted \verb=TPreciseArray=: we still keep key-type associations for
    every entries that we know of, but since this array has been ``polluted'',
    we keep a so--called ``polluted type'' that represent the possible type for
    any undefined key lookup. For example, the following code would induce a
    polluted array: \verb%$a = array('a' => 'b'); $a[$n] = 2;%. The array type
    would be $(a \rightarrow TInt|TString)$ with polluted type $TInt$
